'''
creates GUI for alignment program
...
buttons for:
    crop 
    combine data
    motion align combine with nans
    back projection
'''
import pickle
import numpy as np
...
def motion_align(data):
    ...
    plt.imshow(np.abs(newdata['scan_data']),
               extent=(
                       newdata['range_bins'][0],
                       newdata['range_bins'][-1],
                       newdata['scan_timestamps'][-1] - newdata['scan_timestamps'][0],
                       0),
               aspect='auto')
    ...
    print(len(newdata['motion_timestamps']) == len(newdata['platform_pos']))
    
    r1 = np.sqrt(np.sum(
            (newdata['platform_pos'] - newdata['corner_reflector_pos'][0])**2, 1))
